...
static AYCheckManager *checkManager = nil; // 全局变量
...
- (void)getInfoFromAppStore
{
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (urlResponse.statusCode == REQUST_SUCCEED) {
            ...
            if ([responseDic[@"resultCount"] intValue] == 1) {
                ...
                [userDefault setObject:responseDic[@"version"] forKey:APP_LAST_VERSION];
                [userDefault setObject:responseDic[@"releaseNotes"] forKey:APP_RELEASE_NOTES];
                [userDefault setObject:responseDic[@"trackViewUrl"] forKey:APP_TRACK_VIEW_URL];
                [userDefault setObject:resultDic[@"trackId"] forKey:TRACK_ID];
                // 出现过xcode内的Version 与 appStore 发布版本不一致
...
/**
 * 比较当前版本号是否与沙盒中的版本号相同 如果大于上次版本 YES
 */
- (BOOL)isEqualByCompareLastVersion:(NSString *)lastVersion withCurrentVersion:(NSString *)currentVersion
{
    ...
    for (int index; index < lastVersionArray.count; index ++) {
