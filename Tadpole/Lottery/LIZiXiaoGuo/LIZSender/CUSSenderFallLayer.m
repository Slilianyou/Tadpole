...
- (id)initWithImageNameArray:(NSArray *)imageNameArray
{
    self = [super init];
    if (self) {
        self.imageNameArray = [NSArray arrayWithArray:imageNameArray];
...
- (void)initializeValue
{
    ...
    parentLayer.shadowOpacity = 1.0; // 不透明度
...
- (CAEmitterCell *)createSubLayerContainer
{
    ...
    containerLayer.birthRate   = 10.0; // 每秒发射个数
...
- (CAEmitterCell *)createSubLayer:(UIImage *)image
{
    ...
    cellLayer.emissionRange = 0.5 *M_PI; //  // some variation in angle  角度
    
    cellLayer.spinRange = 0.25 *M_PI; // slow spin 自旋
